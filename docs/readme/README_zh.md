...
如果应用会接收用户或外部服务提供的归档文件，那么在解压前最好先确认其中没有危险路径、链接或不受支持的元素，
并确认归档文件大小和解压后的数据量都处于应用允许的范围内。

这个包既可以在解压前单独检查归档文件，也可以在一次操作中完成检查和提取。
如果归档文件未通过检查，应用会得到明确的原因。

## 这个包会做什么

- 根据文件内容而不是文件扩展名识别格式；
- 检查归档文件中的路径，防止路径逃逸到目标目录之外；
- 拒绝符号链接、硬链接、特殊对象以及不受支持的归档文件元素；
- 限制归档文件最大大小、内部文件和文件夹数量，以及解压后的数据总量；
- 返回结构化的违规列表，便于应用处理；
- 可以通过 `inspect()` 单独检查归档文件，也可以通过 `extract()` 检查并提取；
...
[`ArchiveGuard::inspect()`](../../src/ArchiveGuard.php) 只检查归档文件，不会向磁盘提取文件。
...
以上数值仅用于演示。实际值应根据应用真正预期接收的归档文件大小来设置。
...
[`ArchivePolicy`](../../src/ArchivePolicy.php) 定义归档文件一旦超过就会被拒绝的上限：

- 归档文件文件的最大大小；
- 归档文件内文件和文件夹的最大数量；对于 TAR，一些格式元数据元素也计入同一限制；
- 单个文件解压后的最大大小；
- 所有解压数据的最大总量；
- 可选的“解压后大小与压缩后大小之比”上限。

这些值不存在通用答案：头像上传可接受的归档文件和备份文件可接受的归档文件需要不同限制。
所有参数及规则请参阅[策略参考](../reference/policy-and-errors_zh.md)。

## 违规与错误

如果归档文件格式能够识别，但未通过配置的检查，
...
- **每次提取都需要独立的空目录。** 如果应用使用一个公共目录存放所有归档文件的结果，
  应为每次操作在其中新建一个子目录。当前版本不会把归档文件内容覆盖式解压到已有文件上。
- **不会替换已有文件或目录。** 如果提取过程中目标路径上已经出现对象，操作会报错，
  而不是覆盖它。
- **不会恢复归档文件中的权限、所有者和修改时间。** 会提取文件内容和目录结构，但不会应用
  归档文件中保存的这些元数据。
- **Windows 对文件名有额外限制。** 例如 Windows 不允许 `CON`、`NUL`、以点结尾的名称，
  以及某些包含特殊字符的名称。因此归档文件可能通过通用检查，但在 Windows 上真正开始提取前被拒绝。
