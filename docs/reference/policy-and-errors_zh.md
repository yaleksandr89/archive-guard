...
| 参数 | 限制内容 | 允许值 |
| --- | --- | --- |
| `maxArchiveBytes` | 压缩包文件本身的最大大小 | 大于零的整数 |
| `maxEntries` | 压缩包内文件和文件夹的最大数量；对于 TAR，一些格式元数据元素也计入同一限制 | 大于零的整数 |
...
- `message` — 简短的诊断说明；
- `entryName` — 文件、目录或其他压缩包元素名称；如果违规针对整个压缩包，则为 `null`。
...
| `symlink_entry` | 压缩包中发现符号链接 |
| `hardlink_entry` | TAR 中发现硬链接 |
| `special_entry` | 发现特殊对象，例如设备 |
| `archive_too_large` | 压缩包文件大小超过 `maxArchiveBytes` |
...
| `unsupported_feature` | 压缩包使用了本包不支持的格式功能 |

并不是每个违规都有 `entryName`。例如，压缩包文件总大小超限针对的是整个压缩包，
因此没有单独的元素名称。
...
- 如果压缩包可以读取并通过检查，方法返回 `InspectionResult`，且 `isAccepted() === true`。
- 如果压缩包可以读取但发现违规，方法仍返回 `InspectionResult`，但 `isAccepted()` 为 `false`，
  原因位于 `violations()`。
- 如果文件本身无法打开或正确解析，则抛出 `ArchiveOpenException`。

### `extract()`

- 如果压缩包未通过提取前的必要检查，则抛出 `ArchiveRejectedException`。
