...
[`ArchiveGuard::extract()`](../../src/ArchiveGuard.php) 会先按照传入策略检查归档文件，
只有检查成功后才开始创建文件。
...
目标目录必须**提前存在并且为空**。当前版本不会把归档文件覆盖式解压到已有文件上。

如果应用使用一个公共目录保存所有提取结果，请为每次操作在其中创建新的子目录。
这样不同归档文件的内容不会混在一起，库也可以在开始写入前完整检查目标目录。
...
`extract()` 总会在真正写入前检查归档文件的当前状态，无论之前是否调用过 `inspect()`。
...
5. 写入时，会把每个文件的实际大小和解压数据总量与配置的限制以及检查阶段得到的大小比较。
   如果归档文件开始产生超过允许范围的数据，提取会停止。
6. TAR 和 TAR.GZ 会读取两次：第一次用于检查，第二次用于提取。第二次读取时会把每个元素的顺序、
   路径、类型和大小与第一次检查结果比较。如果两次之间归档文件发生变化，提取会停止。
...
- [`ArchiveRejectedException`](../../src/Exception/ArchiveRejectedException.php) — 归档文件结构有效，
  但没有通过限制或检查；
...
因此同一个归档文件可能在格式层面通过 `inspect()`，却在 Windows 上准备写入时被 `extract()` 拒绝。

## 文件元数据

归档文件除了文件内容外，还可以保存权限、所有者和修改时间。
当前版本会提取文件内容和目录结构，但不会应用归档文件中的这些元数据。
